...
async def get_by_id(assignment_id: str, conn=Depends(get_connection)):
    repo = AssignmentRepository(conn)
    return repo.get_by_id(assignment_id)


@ASSIGNMENT_CONTROLLER.put("/assignments/{assignment_id}")
async def update_by_id(assignment_id: str, new_item: Assignment, conn=Depends(get_connection)):
    repo = AssignmentRepository(conn)
...
async def create(new_assignment: Assignment, conn=Depends(get_connection)):
    repo = AssignmentRepository(conn)
...
async def delete_by_id(assignment_id: str, conn=Depends(get_connection)):
    repo = AssignmentRepository(conn)
