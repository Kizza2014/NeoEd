...
async def get_by_id(class_id: str, conn=Depends(get_connection)):
    repo = ClassroomRepository(conn)
    return repo.get_by_id(class_id)


@CLASSROOM_CONTROLLER.put("/classrooms/{class_id}")
async def update_by_id(class_id: str, new_item: Classroom, conn=Depends(get_connection)):
    repo = ClassroomRepository(conn)
...
async def create(new_classroom: Classroom, conn=Depends(get_connection)):
    repo = ClassroomRepository(conn)
...
async def delete_by_id(class_id: str, conn=Depends(get_connection)):
    repo = ClassroomRepository(conn)
