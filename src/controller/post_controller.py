...
async def get_by_id(post_id: str, conn=Depends(get_connection)):
    repo = PostRepository(conn)
    return repo.get_by_id(post_id)


@POST_CONTROLLER.put("/posts/{post_id}")
async def update_by_id(post_id: str, new_item: Post, conn=Depends(get_connection)):
    repo = PostRepository(conn)
...
async def create(new_post: Post, conn=Depends(get_connection)):
    repo = PostRepository(conn)
...
async def delete_by_id(post_id: str, conn=Depends(get_connection)):
    repo = PostRepository(conn)
