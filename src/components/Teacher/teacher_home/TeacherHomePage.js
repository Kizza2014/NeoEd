      ...
      const fetchClasses = async () => {
        try {
          ...
          if (error.response?.status === 422 || error.response?.status === 401 || error.response?.status === 403) {
            try {
              const fetchNewToken = await axios.post(
                ...
                if (error.response?.status === 422 || error.response?.status === 401 || error.response?.status === 403) {
                  ...
                  alert('Your session has expired or the refresh token is invalid. Please log in again.');
                  navigate("/");
                } else {
                  setError("An error occurred: " + (error.response?.status || "unknown") + " " + error.code);
                }
              }
          } else {
